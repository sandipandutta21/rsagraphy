			...
			if(channels == 2): #stereo
				...
				for z in range(len(data)):
					for q in range(len(data[z])):
						if(data[z][q]<0):
							posdata[z][q] = random.randint(-30000, -100)
						elif(data[z][q]>0):
				...
				for m in range(len(data)):
					for r in range(len(data[m])):
						if(data[m][r]>0):
							negdata[m][r] = random.randint(10, 30000)
						elif(data[m][r]<0):
				...
				for i in range(0, tup[0]):
					for j in range(0, tup[1]):
						if(posdata[i][j]<0 and negdata[i][j] < 0):
							x = negdata[i][j] 
							x = ((pow(x,e)) % n)
							negdata[i][j] = x
						elif(posdata[i][j]>0 and negdata[i][j]>0):
							x = posdata[i][j] 
							x = ((pow(x,e)) % n)
							posdata[i][j] = x
						else:
				...
				for z in range(len(data)):
					if(data[z]<0):
						posdata[z] = random.randint(-32000, -30000)
					elif(data[z]>0):
						posdata[z] = data[z]
					else:
						posdata[z] = 0
				...
				for m in range(len(data)):
					if(data[m]>0):
						negdata[m] = random.randint(30000, 32000)
					elif(data[m]<0):
						negdata[m] = data[m]
					else:
						negdata[m] = 0
				...
				for i in range(len(data)):
					if(posdata[i]<0 and negdata[i]< 0):
						x = negdata[i]
						x = ((pow(x,e)) % n)
						negdata[i] = x
					elif(posdata[i]>0 and negdata[i]>0):
						x = posdata[i]
						x = ((pow(x,e)) % n)
						posdata[i]= x
					else:
			...
			if(channel1 == 2): 
				...
				for i1 in tqdm(range(len(data))):
					for j1 in (range(len(data[i1]))):
						if(data[i1][j1]<0 or data1[i1][j1] < 0):
							x1 = data1[i1][j1] 
							x1 = powmod(x1, d, n)   #= ((pow(x1,16971)) % 25777)
							x1 = x1 - n
							data[i1][j1] = x1
						elif(data[i1][j1]>0 or data[i1][j1]>0):
							x1 = data[i1][j1] 
							x1 = powmod(x1, d, n)   #= ((pow(x1,16971)) % 25777)
							data[i1][j1] = x1
						else:
				...
				for i1 in tqdm(range(len(data))):
					if(data[i1]<0 or data1[i1] < 0):
						x1 = data1[i1] 
						x1 = powmod(x1, d, n)  
						x1 = x1 - n
						data[i1] = x1
					elif(data[i1]>0 or data1[i1]>0):
						x1 = data[i1]
						x1 = powmod(x1, d, n)   
						data[i1] = x1
					else:
				...
				data = numpy.array(data).astype(numpy.int16)
				print('The original data: \n', +data)
				scipy.io.wavfile.write('Decrypted.wav', FS1, data)
...
btn_8_encrypt = Button( audio_window, text = "8bit Audio Encryption", command = eight_encrypt)
